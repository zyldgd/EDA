...
## 3.6 仿真和验证
使用 modelsim 进行仿真，在仿真文件 [test bench](code/testBench_lockeddoor.v) 中测试了输入密码正确、输入密码错误、输入中断和修改密码等操作。在该文件中，定义了一个按键任务块，按键时间间距#1000持续#1000，用来模拟按键按压。
